...
class Main {
  ...
  byte [] daysToPay = new byte[1];
  
  for (float amt : daysToPay){
    amt=amtPerDay * val;
    System.out.println("the total amount the debtor is to pay is $"+ amt);
  ...
  System.out.println("the number of days paid for is "+noOfDaysPaid);
